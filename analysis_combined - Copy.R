...
conditions_to_compare <- c("midSFor_lowSBrood", "midSFor_midSBrood", "midSFor_highSBrood", "highSFor_lowSBrood", "highSFor_midSBrood", "highSFor_highSBrood")  # Add the conditions you want to compare
frac_dot <- 0.05
num_reactnorms <- 25
last_time <- 100000
...
for (condition in conditions_to_compare) {
  ...
  if (condition %in% unique(mapping$condition)) {
    ...
    if (nrow(subset_data) > 1) {
      ...
      #### Histogram ####
      # Create a histogram plot for the last 100,000 units of each replicat
      
      # Loop through each replicate
      for (rep_val in unique(subset_data$rep)) {
        rep_data <- subset_data[subset_data$rep == rep_val, ]
        last_100k_data <- rep_data[rep_data$gtime >= (max(rep_data$gtime) - 100000), ]
...
ggsave(paste0(bigBosstitle,"_DispForage.png"), final_combined_plot, height = 15, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
...
ggsave(paste0(bigBosstitle,"_Additional.png"), final_combined_plot, height = 20, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
...
ggsave(paste0(bigBosstitle,"_BetaVals.png"), final_combined_plot, height = 20, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
...
if (dum_count != 0) {
  ...
  ggsave(paste0(bigBosstitle,"_ReactNorms.png"), final_grid_plot, height = num_replicates*5, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
...
if (dum_count != 0) {
  ...
  final_grid_plot <- grid.arrange(grobs = t(grid), ncol = num_cols)
  # Add a title to the final_grid_plot
  ggsave(paste0(bigBosstitle,"_Lifespans.png"), final_grid_plot, height = num_replicates*5, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
...
if(dum_count != 0){
  ...
  final_grid_plot <- grid.arrange(grobs = t(grid), ncol = num_cols)
  # Add a title to the final_grid_plot
  ggsave(paste0(bigBosstitle,"_numChoices.png"), final_grid_plot, height = num_replicates*5, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
...
if(dum_count != 0){
  ...
  # Add a title to the final_grid_plot
  ggsave(paste0(bigBosstitle,"_NestFemales.png"), final_grid_plot, height = num_replicates*5, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
  ggsave(paste0(bigBosstitle,"_NestFemLarvae.png"), final_gridlarv_plot, height = num_replicates*5, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
