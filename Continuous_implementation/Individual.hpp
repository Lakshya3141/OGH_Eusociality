...
public:
    ...
    Individual(const diploid_genome& genome_mum, const haploid_genome& genome_dad); // female COULD REMOVE AND ADD IT TO POPULATION CLASS
    ...
    void forage(const double& mean = dForagingMean, const double& SD = dForagingSD);    // REMOVE THIS AND ADD IT TO NEST FUNCTION
    void feed(const double& food);                                                      // MAYBE add this to nest as well?
    bool check_mature();                                                                // ALSO maybe at the nest level
