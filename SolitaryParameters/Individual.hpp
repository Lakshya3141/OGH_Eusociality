...
public:
    ...
    Individual(const int id, const Individual<2>& mum, const params& p);   // male and female?
...
template <>
Individual<1>::Individual (const int id) : ind_id(id), t_death(0.0) { } //JK: initialiser lists are computationally faster
...
template <>
Individual<2>::Individual (const int id, const Individual<2>& mum, const params& p) : nest_id(mum.nest_id), is_larvae(true), ind_id(id), mom_id(mum.ind_id) {
...
bool Individual<2>::check_disperser(){
    if (bernoulli(logistic(phenotype_dispersal))) { //JK: I logistically transform the dispersal phenotypic value. then it is between 0 and 1
        is_disperser = true;
        return true;
    }
    else return false;
