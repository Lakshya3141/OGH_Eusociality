...
bigBosstitle <- "lowBrood"

# Step 1: Classifying data
mapping <- read.csv("mapping_condition.csv")
conditions_to_compare <- c("lowBrood_lowFor", "lowBrood_midFor", "lowBrood_highFor")  # Add the conditions you want to compare
...
for (condition in conditions_to_compare) {
  ...
  if (condition %in% unique(mapping$condition)) {
    ...
    for (replic in unique(subset_data$rep)) {
      ...
      p_lifespan <- ggplot(all_individuals, aes(x = lifespan)) +
        geom_histogram(fill = "lightblue", color = "black") +
        theme_minimal() +
        labs(title = paste0(condition, " : ", replic), x = "Lifespan") +
        theme(legend.position = "none") +
        geom_vline(xintercept = mean(all_individuals$lifespan, na.rm = TRUE), col = "red", lwd = 2) +
        annotate("text", x = mean(all_individuals$lifespan, na.rm = TRUE), y = 10,
                 label = paste("Average Lifespan:", round(mean(all_individuals$lifespan, na.rm = TRUE), 2)), col = "red")
      
      p_tasks <- ggplot(all_individuals, aes(x = num_choices)) +
        geom_histogram(fill = "lightblue", color = "black") +
        theme_minimal() +
        labs(title = paste0(condition, " : ", replic), x = "Lifespan") +
        theme(legend.position = "none") +
        geom_vline(xintercept = mean(all_individuals$num_choices, na.rm = TRUE), col = "red", lwd = 2) +
        annotate("text", x = mean(all_individuals$num_choices, na.rm = TRUE), y = 10,
                 label = paste("Average num_choices:", round(mean(all_individuals$num_choices, na.rm = TRUE), 2)), col = "red")
...
ggsave(paste0(bigBosstitle,"_DispForage.png"), final_combined_plot, height = 10, width = length(conditions_to_compare)*5, units = "in", dpi = 300)
