...
// Feed checking  implemented here
...
void Nest::task_check(const Eigen::MatrixXd& tmp_matrix, Individual<2>& current, const params& p) {
    ...
    double dum_cast = static_cast<double>(MaxLarvaeSpline);
    double foraging_propensity = evaluate_spline2d(tmp_matrix, current.choice_phenotype, n_resolution, total_larvae/dum_cast);
    ...
    if (bIsTimeUnit) {
        ...
        double dum_brood = normal(p.dBroodingTime, p.dBroodingTimeSD);
        if (dum_fora < 0) dum_fora = 0;
        if (dum_brood < 0) dum_brood = 0;
        current.t_next = current.t_next + (current.is_foraging ? dum_fora : dum_brood);
    }
