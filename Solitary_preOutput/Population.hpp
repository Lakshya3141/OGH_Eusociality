    ...
    for (int event = 0; event < max_events; ++event) {
        ...
        if (current.is_alive) { // If individual survived till now
            if (current.is_foraging) {  // And old task is foraging
                ...
                if (std::get<0>(index) == true) {       // If feed successful
                    if (std::get<1>(index) == false) {  // if male
                        ...
                        if (nests[cnestid].larval_males[std::get<2>(index)].check_mature(gtime, p)) {
                            ...
                            // size_t old_lmales = nests[cnestid].larval_males.size(); // TST1
                            // size_t old_amales = adult_males.size(); // TST1
                            ...
                            // std::cout << "FOR Male ID: " << adult_males[adult_males.size()-1].ind_id
                            // << " MATURED Adult from " << old_amales << " to " << adult_males.size()
                            // << " Larvae From " << old_lmales << " to " << nests[cnestid].larval_males.size() << std::endl; // TST1
                        } else {
                            // std::cout << "FOR Male ID: " << nests[cnestid].larval_males[std::get<2>(index)].ind_id 
                            // << " No Mature " << std::endl; // TST2
                        }
                    } else { // if female
                        // Check mature
                        if (nests[cnestid].larval_females[std::get<2>(index)].check_mature(gtime, p)) {
                            ...
                            if (nests[cnestid].larval_females[std::get<2>(index)].check_disperser()) {
                                ...
                                if (std::get<0>(empty) == true) {       // If empty nest found
                                    ...
                                    // int confirm_empty = nests[empty_index].adult_females.size(); // TST3
                                    ...
                                    // << " NewAFem size from " << confirm_empty << " to " << nests[empty_index].adult_females.size() <<std::endl; // TST3
                                } else {
                                    // If not disperser
                                    // size_t old_lfemales = nests[cnestid].larval_females.size(); // TST4
                                    // double old_lbodyzize = nests[cnestid].larval_females[std::get<2>(index)].body_size; // TST4
                                    ...
                                    // << old_lfemales << " to " << nests[cnestid].larval_females.size() << std::endl; // TST4
                                }
                            } else { // else if not disperser
                                // size_t old_lfemales = nests[cnestid].larval_females.size(); // TST5
                                // double old_bodysize = nests[cnestid].larval_females[std::get<2>(index)].body_size; // TST5
                ...
                // size_t num_lmale = nests[cnestid].larval_males.size(); // TST7
                // size_t num_lfemale = nests[cnestid].larval_females.size(); // TST7
                ...
                // << " LMale From " << num_lmale <<" to " << nests[cnestid].larval_males.size() << std::endl; // TST7
            }
