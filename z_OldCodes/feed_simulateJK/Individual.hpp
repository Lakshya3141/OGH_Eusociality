...
public:
    ...
    //void forage(const double& mean = dForagingMean, const double& SD = dForagingSD); //JK: can be deleted here
    ...
    //double resources = 0.0;     // resources brought back in one foraging trip //JK: can be deleted here
...
//} //JK: could also be deleted actually
...
bool Individual<2>::task_check() {
    ...
    is_foraging = tmp1;
    t_prev = t_next;
    //t_next = t_next + is_foraging ? dForagingTime : dBroodingTime; //JK: got a really important warning here: Operator '?:' has lower precedence than '+'; '+' will be evaluated first
    t_next = t_next + (is_foraging ? dForagingTime : dBroodingTime); //JK: this maybe fixes it but not 100% sure
...
bool Individual<1>::check_mature(double& birth_time){
    ...
    else return false; //JK: added this to remove an error message
}
