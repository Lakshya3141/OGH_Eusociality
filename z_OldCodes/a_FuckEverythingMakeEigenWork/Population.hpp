...
public:
    ...
    Eigen::MatrixXd basis_matrix_choice{ make_spline2d(n_knots, n_resolution, 0.0, p.dMaxSplineLarvae) };
...
void Population::simulate_tst(const std::vector<std::string>& param_names) {
    ...
    di_file << "gtime,pertime_foraging,num_foraging,num_choices,nest_id" << std::endl;
    ...
    while (gtime < max_gtime_evolution) {
        ...
        if (current.is_alive) { // If individual survived till now
            ...
            if (bernoulli(frac_ind_dot_plot)) {
                ...
                auto dummy = std::make_tuple(current.t_next, pertime_foraging, current.num_foraging, current.num_choices, current.nest_id);
