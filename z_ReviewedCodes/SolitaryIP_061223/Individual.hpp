...
template <>
Individual<1>::Individual (const int id) : t_death(0.0), ind_id(id)  { }
...
Individual<2>::Individual (const int id, const Individual<2>& mum, const params& p) : nest_id(mum.nest_id), is_larvae(true), ind_id(id), mom_id(mum.ind_id), dad_id(mum.mate_id) {
    ...
    // IP: first check if mom actally has sperm?
    genome[0] = mum.sperm;
    genome[1].genes_dispersal = mum.genome[bernoulli()].genes_dispersal;

    // IP: so full recombination between intercept and slope genes
...
void Individual<2>::survival(const params& p) {
    ...
    // IP: why XOR? OR is faster because it doesn't evaluate the second expression if the first evaluates as TRUE
...
bool Individual<1>::check_mature(double& birth_time, const params& p) {
    if (bernoulli(logistic(body_size, p.dLarvaIntercept, p.dLarvaSlope))) {
        // IP: so the bigger the more likely to mature (seeing as slope=-5)

        is_larvae = false;
        t_birth = birth_time;
        // inverse transform sampling
        t_death = birth_time + ( - log(1.0 - uni_real()) / p.dMaleLambda );
        return true;
    }
    return false;
}

// IP: why is maturation "stochastic"?
