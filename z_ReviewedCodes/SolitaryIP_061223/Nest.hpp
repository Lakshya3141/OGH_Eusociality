...
// 
// IP: not clear (yet) what the returned index is needed for; guess we'll see!
...
// IP: why is this a member function of Nest instead of Individual?
void Nest::task_check(Individual<2>& current, const params& p) {

    // IP: so foraging choice depends on nr of larvae according to logistic reaction norm
    ...
    // IP: why add noise here?
}

// Function to search by indidivual ID in a nest and return index
// IP: I wonder what this is for ...
// IP: shouldn't the id argument be constant?
size_t Nest::findFemaleIndexById(unsigned int id) {
    auto it = std::find_if(adult_females.begin(), adult_females.end(), [id](const Individual<2>& ind) {
        ...
        // IP: ind.ind_id and id not same type
    });
