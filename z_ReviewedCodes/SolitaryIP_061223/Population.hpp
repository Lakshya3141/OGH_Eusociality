...
struct track_time {
    ...
    // event constructor
    // IP: initialized with initializer list instead of in body
    // I also made the types agree between track_time and Individual
...
// IP: but don't you want the individual with the smallest time?

// initialise population function
void Population::initialise_pop() {
    // clear vectors
    // IP: seems unnecessary at initialization
    adult_males.clear();
    nests.clear();
    empty_nests.clear();

    // IP: maybe reserve space here. Can save time.
    ...
    for(unsigned int i=0; i < max_nests; ++i) {
        ...
        // and might be faster although I
    }
}
...
void Population::simulate_tst() {
    ...
    for (int event = 0; event < max_events; ++event) {
        ...
        if (event % check_dead_males == 0) { // currently 1000
            removeDeadMales();
        }
        ...
        // IP: why copy instead of reference?
        ...
        if (current.is_alive) { // If individual survived till now
                    ...
                    if (std::get<1>(index) == false) {  // if male
                        ...
                        if (f.check_mature(gtime, p)) {
                            ...
                            if (f.check_disperser()) {
                                ...
                                // Get Empty nest ID and its index in nests vector
                                ...
                                if (std::get<0>(empty) == true) {       // If empty nest found
                                    ...
                                    // IP: why nor use remove_from_vec?
            ...
            } else { // old task is reproducing instead of foraging

                // reproduce
                nests[cnestid].reproduce(nests[cnestid].adult_females[current_index], individual_id_counter, p);
                ++individual_id_counter;
                // IP: why increase counter? Because a single offspring has been produced?
            }
...
std::tuple<bool, size_t> Population::choose_RndmMale(){
    if (adult_males.size() != 0) {
        ...
        // IP: why +1? Now it will never return index 0
...
void Population::mate(Individual<2>& female){
    ...
    // IP: and what if there are no males?
}
...
std::tuple<bool, unsigned int> Population::update_emptyNests() {
    empty_nests.clear();        // clear empty_nests vector [IP: that's a vector<size_t>]
...
void Population::removeDeadMales() {
    ...
    // IP: indeed, can use erase_if instead of erase-remove_if. Requires c++20
}
